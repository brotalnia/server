...
bool ChatHandler::HandleDebugSendNextChannelSpellVisualCommand(char *args)
{
    ...
    for (id = uiPlayId + 1; id <= sSpellStore.GetMaxEntry(); id++)
    {
    ...
    if (id && id <= sSpellStore.GetMaxEntry())
    {
    ...
    PSendSysMessage("Spell Id range 0 to %u, -1 to stop animation", sSpellStore.GetMaxEntry());
    return true;
}
...
bool ChatHandler::HandleSendSpellChannelVisualCommand(char *args)
{
    ...
    if (uiPlayId && uiPlayId <= sSpellStore.GetMaxEntry())
    {
        WorldPacket data(MSG_CHANNEL_START, (4 + 4));
    ...
    PSendSysMessage("Spell Id range 1 to %u, 0 to stop animation", sSpellStore.GetMaxEntry());
    return true;
}
