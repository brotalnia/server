...
SpellStore::~SpellStore()
{
    for (auto it = m_DBSpellEntryMap.begin(); it != m_DBSpellEntryMap.end(); ++it)
        delete *it;
}

SpellStore& SpellStore::Instance()
{
    static SpellStore spellStore;
    return spellStore;
}

void SpellStore::Load()
{
    m_DBSpellEntryMap.clear();
    ...
    if (!result)
    {
        sLog.outString(">> Loaded 0 spell templates. DB table `spell_template` is empty.");
        return;
    }
    auto fields = result->Fetch();
    m_maxEntry = fields[0].GetUInt32() + 1;
    delete result;

    // Getting the number of spells.
    result = WorldDatabase.Query("SELECT COUNT(*) FROM spell_template");
    ...
    fields = result->Fetch();
    m_recordCount = fields[0].GetUInt32();
    delete result;
    ...
    m_DBSpellEntryMap.resize(m_maxEntry, nullptr);

    do
    {
        fields = result->Fetch();

        DBSpellEntry* spell = new DBSpellEntry();
        ...
        spell->SpellNameFlag = fields[128].GetUInt32();
        ...
        m_DBSpellEntryMap[spellId] = spell;
    ...
    sLog.outString(">> Loaded %u base spell templates.", m_recordCount);
    sLog.outString();
}
...
void SpellMgr::LoadSpells()
{
    ...
    {
        if (DBSpellEntry const* spellEntry = sSpellStore.LookupEntry(i))
        {
